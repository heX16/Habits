...
### Экземпляр и соединения

В Flask при старте создаётся **один** экземпляр `HabitsDatabase` (`get_database` в `flask_backend_core.py`); он передаётся во все API-хендлеры. Это не долгоживущее SQLite-соединение, а обёртка над путём к файлу: каждый метод сам делает `connect()` → операция → `close()`.

Для этого приложения надёжность важнее оптимизации мелких записей: write-behind / очередь на бэкенде не используется — каждое изменение сразу открывает файл и коммитится в БД. (Батчинг обновлений статусов есть только на фронте: `UPDATE_QUEUE_INTERVAL_MS`.)
